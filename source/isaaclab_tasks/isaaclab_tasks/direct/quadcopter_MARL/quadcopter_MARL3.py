...
class QuadcopterEnvCfgMARL3(DirectMARLEnvCfg):
    ...
    #action_space = 4
    ### ------------------------------------------
    possible_agents = ["T1", "T2", "T3", "T4"]
    action_spaces = {"T1": 1, "T2": 1, "T3": 1, "T4": 1}
    ### ------------------------------------------
    #observation_space = 12
    # observation_spaces = {"Translation": 18, "Yaw": 12}
    observation_spaces = {"T1": 15, "T2": 15, "T3": 15, "T4": 15}
    state_space = -1
    debug_vis = True
...
class QuadcopterEnvMARL3(DirectMARLEnv):
    ...
    def __init__(self, cfg: QuadcopterEnvCfgMARL3, render_mode: str | None = None, **kwargs):
        ...
        self.f_check += 10.0
        self.fM = torch.zeros(self.num_envs, 4, device=self.device)
    ...
    def _pre_physics_step(self, actions: torch.Tensor):
        ...
        act1 = actions["T1"].clone().clamp(0.0, 0.16)
        act2 = actions["T2"].clone().clamp(0.0, 0.16)
        act3 = actions["T3"].clone().clamp(0.0, 0.16)
        act4 = actions["T4"].clone().clamp(0.0, 0.16)
        ...
        self.Forces[:, 0] = act1.squeeze()
        self.Forces[:, 1] = act2.squeeze()
        self.Forces[:, 2] = act3.squeeze()
        self.Forces[:, 3] = act4.squeeze()
        for env in range(self.num_envs):
            self.fM[env, :] = torch.matmul(self.M, self.Forces[env, :])

        self._thrust[:, 0, :2] = 0.0
        self._thrust[:, 0, 2] = self.fM[:, 0]
        self._moment[:, 0, 0] = self.fM[:, 1]
        self._moment[:, 0, 1] = self.fM[:, 2]
        self._moment[:, 0, 2] = self.fM[:, 3]
    ...
    def _get_rewards(self) -> dict[str, torch.Tensor]:
        ...
        return {"T1": 0.25 * reward, "T2": 0.25 * reward, "T3": 0.25 * reward, "T4": 0.25 * reward}
    ...
    def _get_dones(self) -> tuple[dict[str, torch.Tensor], dict[str, torch.Tensor]]:
        ...
        self.bonus = torch.where(self.check_count > 300.0, 200, 0.0)
