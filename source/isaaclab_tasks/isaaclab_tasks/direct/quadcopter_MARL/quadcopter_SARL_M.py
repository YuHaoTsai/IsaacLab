...
class QuadcopterEnvWindowSARLM(BaseEnvWindow):
    ...
    def __init__(self, env: QuadcopterEnvSARLM, window_name: str = "IsaacLab"):
...
class QuadcopterEnvCfgSARLM(DirectMARLEnvCfg):
    ...
    observation_spaces = {"main": 12, "wind": 9}
    state_space = -1
    debug_vis = True
...
class QuadcopterEnvSARLM(DirectMARLEnv):
    ...
    def _pre_physics_step(self, actions: torch.Tensor):
        ...
        self._thrust[:, 0, 2] = self.cfg.thrust_to_weight * self._robot_weight * (act1[:, 0] + 1.0) / 2.0 + (0.01 * act2[:, 0])
        self._moment[:, 0, :] = self.cfg.moment_scale * (act1[:, 1:]) + (0.001 * act2[:, 1:])
